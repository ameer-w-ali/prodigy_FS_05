...
type ComentWithUser = Comment & { user: User };

const CommentList = ({ comments, postId }: { comments: ComentWithUser[]; postId: number }) => {

    const { user } = useUser()
    const [commentState, setCommentState] = useState(comments)
    const [desc, setDesc] = useState("")

    // function to use our comment server action
    const add = async () => {
        if(!user || !desc) return;
    ...
    //optimistic hook for comments
    const [optimisticComments, addOptimisticComment] = useOptimistic(commentState,
        // previous comments + add the new one
        (state, value: ComentWithUser) => [...state, value]
    )
            ...
            <div className="">
                ...
                {optimisticComments.map(comment => (
                    <div className="flex gap-4 justify-between mt-6" key={comment.id}>
                        {/* Avatar */}
                        <Image
                            src={comment.user.avatar || "/noAvarat.png"}
                            alt=""
                            width={40}
                            height={40}
                            className="w-8 h-8 rounded-full" />
